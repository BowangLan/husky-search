    ...
    def update_courses(self, 
        ...
        try:
            for i in range(0, len(courses), self.batch_size):
                ...
                percentage = min((shifted_i + self.batch_size) / total_courses * 100, 100)
                print(
...
def update_courses_title(cursor, courses: List[Dict[str, Any]]):
    ...
    cursor.executemany(
        sql,
        [(course["title"], course["code"]) for course in courses],
    )
...
def transform_title_data(course: Dict[str, Any]) -> Dict[str, Any]:
    ...
    full_code = f"{course['subject']} {course['number']} "
    return {
        "code": course["code"],
        "title": course["name"].split(" (")[0].strip().replace(full_code, "")
    }
